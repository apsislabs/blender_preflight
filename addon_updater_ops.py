...
updater.addon = "blender_preflight"
...
class addon_updater_install_manually(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        if self.error != "":
            ...
            col.scale_y = 0.7
            col.label(
                "There was an issue trying to auto-install", icon="ERROR")
            col.label(
                "Press the download button below and install", icon="BLANK1")
...
def post_update_callback(res=None):
    ...
    if res == None:
        ...
        if updater.verbose:
            print("{} updater: Running post update callback".format(
                updater.addon))
...
def update_settings_ui(self, context):
    ...
    if updater.error != None:
        ...
        split.scale_y = 2
        split.operator(
            addon_updater_check_now.bl_idname, text="", icon="FILE_REFRESH")
        ...
        split.scale_y = 2
        split.operator(
            addon_updater_end_background.bl_idname, text="", icon="X")
    ...
    if updater.manual_only == False:
        ...
        if updater.include_branches == True and len(
...
def register(bl_info):
    ...
    updater.user = "apsislabs"

    # choose your own repository, must match git name
    updater.repo = "blender_preflight"
