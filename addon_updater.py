...
class Singleton_updater(object):
    ...
    def check_interval(self):
        return (self._check_interval_enable, self._check_interval_months,
            ...
            for file in files:
                ...
                if os.path.isfile(destFile):
                    ...
                    if replaced:
                        ...
                        if self._verbose:
                            print(
                                "Overwrote file " + os.path.basename(destFile))
                    else:
                        if self._verbose:
                            print("Pattern not matched to " + os.path.basename(
                                destFile) + ", not overwritten")
                else:
    ...
    def check_for_update(self, now=False):
        ...
        if len(self._tags) == 0:
            ...
            if len(self._tags) == n:
                ...
                link = self._tags[0][
                    "zipball_url"]  # potentially other sources
            else:
                link = self._tags[n][
                    "zipball_url"]  # potentially other sources
    ...
    def past_interval_timestamp(self):
        ...
        if "last_check" not in self._json or self._json["last_check"] == "":
            ...
            offset = timedelta(
                days=self._check_interval_days +
            ...
            if delta.total_seconds() > 0:
                if self._verbose:
                    print("{} Updater: Time to check for updates!".format(
                        self._addon))
                return True
            else:
                if self._verbose:
                    print(
                        "{} Updater: Determined it's not yet time to check for updates".
                        format(self._addon))
                return False
    ...
    def start_async_check_update(self, now=False, callback=None):
        ...
        check_thread = threading.Thread(
            target=self.async_check_update, args=(
                now,
                callback,
            ))
...
class GithubEngine(object):
    ...
    def form_branch_url(self, branch, updater):
        return "{}{}{}".format(
            self.form_repo_url(updater), "/zipball/", branch)
...
class GitlabEngine(object):
    ...
    def form_branch_url(self, branch, updater):
        ...
        return "{}{}{}".format(
            self.form_repo_url(updater), "/repository/archive.zip?sha=",
            branch)
